...
// async function animateSwap(container, i, j) {
//   const bars = container.children;
//   if (!bars[i] || !bars[j]) return;

//   const barWidth = bars[i].offsetWidth + 6; // 6px = your current gap

//   // Prepare bars
//   bars[i].style.transition = 'transform 0.3s ease';
//   bars[j].style.transition = 'transform 0.3s ease';

//   // Translate them
//   bars[i].style.transform = `translateX(${barWidth}px)`;
//   bars[j].style.transform = `translateX(${-barWidth}px)`;
...
// async function bubbleSort(arr, render, container) {
...
//       await sleep();
//       if (arr[j] > arr[j + 1]) {
//   // Animate visually before swap
//   await animateSwap(container, j, j + 1);
...
//       case "bubble":
//         await bubbleSort(arr, renderArray, arrayContainer);
//         break;
//       case "selection":
...
//         case "bubble":
//           await bubbleSort(arrDuo, renderArrayDuo, arrayContainerDuo);
...
async function animateSwap(container, i, j) {
  const bars = container.children;
  if (!bars[i] || !bars[j]) return;
  const barWidth = bars[i].offsetWidth + 6;
  bars[i].style.transition = 'transform 0.3s ease';
  bars[j].style.transition = 'transform 0.3s ease';
  bars[i].style.transform = `translateX(${barWidth}px)`;
  bars[j].style.transform = `translateX(${-barWidth}px)`;
...
function updateAlgoLabels() {
  algoLabel1.textContent = algoSelect.options[algoSelect.selectedIndex].text;
  algoLabel2.textContent = duoAlgoSelect.options[duoAlgoSelect.selectedIndex].text;
}

algoSelect.onchange = () => duoMode && updateAlgoLabels();
duoAlgoSelect.onchange = () => duoMode && updateAlgoLabels();
...
duoSwitch.onchange = function () {
  ...
  duoDivider.style.display = duoMode ? "block" : "none";
  algoLabels.style.display = duoMode ? "flex" : "flex";
  algoLabels2.style.display = duoMode ? "flex" : "none";
  if (duoMode) {
    arrDuo = [...arr];
    renderArrayDuo();
    updateAlgoLabels();
...
async function bubbleSort(arr, render, container) {
  ...
  render([], Array.from({ length: arr.length }, (_, i) => i));
}

async function selectionSort(arr, render, container) {
  let sorted = [];
  for (let i = 0; i < arr.length; i++) {
    let minIdx = i;
    for (let j = i + 1; j < arr.length; j++) {
    ...
    if (i !== minIdx) {
      await animateSwap(container, i, minIdx);
      [arr[i], arr[minIdx]] = [arr[minIdx], arr[i]];
    }
    sorted.push(i);
    render([i, minIdx], sorted);
...
async function insertionSort(arr, render, container) {
  let sorted = [];
  for (let i = 1; i < arr.length; i++) {
    let key = arr[i];
    let j = i - 1;
...
async function quickSortHelper(arr, l, r, render, sorted, container) {
  if (l >= r) return;
  let pivot = arr[r];
  let i = l;
  for (let j = l; j < r; j++) {
    ...
    await sleep();
    if (arr[j] < pivot) {
      await animateSwap(container, i, j);
      [arr[i], arr[j]] = [arr[j], arr[i]];
      render([i, j], sorted);
      await sleep();
      i++;
  ...
  await animateSwap(container, i, r);
  [arr[i], arr[r]] = [arr[r], arr[i]];
  sorted.push(i);
  render([i], sorted);
  await sleep();
  await quickSortHelper(arr, l, i - 1, render, sorted, container);
  await quickSortHelper(arr, i + 1, r, render, sorted, container);
}

async function quickSort(arr, render, container) {
  let sorted = [];
  await quickSortHelper(arr, 0, arr.length - 1, render, sorted, container);
  render([], Array.from({ length: arr.length }, (_, i) => i));
}
...
visualizeBtn.onclick = async function () {
  ...
  if (!duoMode) {
    switch (algoSelect.value) {
      case "bubble": await bubbleSort(arr, renderArray, arrayContainer); break;
      case "selection": await selectionSort(arr, renderArray, arrayContainer); break;
      case "insertion": await insertionSort(arr, renderArray, arrayContainer); break;
      case "merge": await mergeSort(arr, renderArray); break;
      case "quick": await quickSort(arr, renderArray, arrayContainer); break;
    }
  } else {
    arr = [...arrDuo];
    arrDuo = [...arr];
    ...
    let p2 = (async () => {
      switch (duoAlgoSelect.value) {
        case "bubble": await bubbleSort(arrDuo, renderArrayDuo, arrayContainerDuo); break;
        case "selection": await selectionSort(arrDuo, renderArrayDuo, arrayContainerDuo); break;
        case "insertion": await insertionSort(arrDuo, renderArrayDuo, arrayContainerDuo); break;
        case "merge": await mergeSort(arrDuo, renderArrayDuo); break;
        case "quick": await quickSort(arrDuo, renderArrayDuo, arrayContainerDuo); break;
      }
    })();
    await Promise.all([p1, p2]);
  }
